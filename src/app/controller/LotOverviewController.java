...
public class LotOverviewController
{
	...
	@FXML
	private TableColumn<Lot, String> nameColumn;
	@FXML
	private TableColumn<Lot, String> categoryColumn;
	@FXML
	private TableColumn<Lot, String> boxColumn;
	@FXML
	...
	private void handleNewLot()
	{
		...
		if(okClicked) app.getLots().add(tempLot);
	}
	...
	private void handleNewLotAs()
	{
		...
		if(selectedLots.size() == 1)
		{
			...
			if(okClicked) app.getLots().add(lot);
		}
		else
		{
			showLotAlert();
	...
	private void showLotDetails(Lot lot)
	{
		if(lot != null)
		{
			...
			if(lot.getImages().size() > 0)
			{
				...
				pic1.setImage(new Image("file:" + lot.getImages().get(0), 430, 370, true, false));
			}
			else
			{

...
public class CategoryService
{
	...
	private final String filePath = "src/app/resources/categories.xml";
	...
	public ObservableList<Category> findCategories(String categoryName)
	{
		...
		categories.forEach(category -> {
			if(category.getName().equals(categoryName)) findCategory.add(category);
		});
		return findCategory;
	}
	...
	public int findMeshokCategory(String categoryName)
	{
		...
		categories.forEach(category -> {
			if(category.getName().equals(categoryName)) findCategory.setMeshokCategory(category.getMeshokCategory());
		});
	...
	private void loadCategories()
	{
		try
		{
			if(fileNotExists()) throw new FileNotFoundException(filePath);
			...
			categories.clear();
			if(wrapper.getCategories() != null) categories.addAll(wrapper.getCategories());
		}

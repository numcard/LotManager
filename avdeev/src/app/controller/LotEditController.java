...
public class LotEditController
{
    ...
    public void setLot(Lot lot)
    {
        ...
        for(int i = 0; i < pics.size(); i++)
        {
            if(lotImages.size() > i)
            {
                ...
                pics.get(i).setImage(new javafx.scene.image.Image(IMG_FILE_PART + lotImages.get(i), 130, 130, false, false));
            }
            else
            {
    ...
    private void handleUploadImages()
    {
        ...
        for(int i = 0; i < pics.size(); i++)
        {
            ...
            {
                pics.get(i).setVisible(true);
                pics.get(i).setImage(new javafx.scene.image.Image(IMG_FILE_PART + files.get(i).getAbsolutePath(), 130, 130, false, false));
            }
            else
                pics.get(i).setVisible(false);
        }

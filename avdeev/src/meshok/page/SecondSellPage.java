...
class SecondSellPage
{
    ...
    private By iFrameLocator =
            By.xpath("/html/body/form/table[2]/tbody/tr/td/table/tbody/tr[5]/td/iframe");    // iFrame изображений
    ...
    SecondSellPage(WebDriver driver, Lot lot)
    {
        ...
        try
        {
            // Проверка что описание(статическое)
            try
            {
                waitElement(descriptionLocator);
            }
            catch(Exception e)
            {
                if(driver.findElements(dynamicDescriptionLocator).size() > 0)
                    driver.findElement(dynamicDescriptionLocator).click();
            }

            waitElements(nameLocator, priceLocator, bestOfferLocator);
            ...
            new Select(driver.findElement(categoryLocator)).selectByValue(category2key(lot.getCategory()));
    ...
    private String category2key(String category) throws Exception
    {
        ...
        if(id == 0)
            throw new Exception("Категория на найдена");
